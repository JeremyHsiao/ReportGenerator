...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        // Open existing excel
        private Excel.Application OpenOridnaryExcel(string filename)
        {
        ...
        private Excel.Application OpenPreviousExcel(string filename)
        {
        ...
        private void ListSheets(Excel.Application curExcel)
        {
            int index = 0;
        ...
        private Worksheet Find_Worksheet(Excel.Application curExcel, string sheet_name)
        {
            Worksheet ret = null;
        ...
        private Dictionary<string, int> CreateTableColumnIndex(Worksheet bug_worksheet, int naming_row)
        {
        ...
        private Dictionary<string, List<StyleString>> CreateBugList(Worksheet bug_worksheet)
        {
        ...
        private Dictionary<string, List<StyleString>> ProcessJiraBugFile(string buglist_filename)
        {
            Dictionary<string, List<StyleString>> myBug_list = new Dictionary<string, List<StyleString>>();

            // Open excel (read-only & corrupt-load)
            Excel.Application myBugExcel = OpenPreviousExcel(buglist_filename);
            if (myBugExcel != null)
            {
                // Find bug worksheet and generate list of bug description string
                Worksheet WorkingSheet = Find_Worksheet(myBugExcel, sheet_BUG_General_Result);
        ...
        private List<StyleString> ExtendIssueDescription(string links_str, Dictionary<string, List<StyleString>> bug_list)
        {
        ...
        private void ProcessTCJiraExcel(string tclist_filename)
        {
            // Open excel (read-only & corrupt-load)
            Excel.Application myTCExcel = OpenPreviousExcel(tclist_filename);
            //Excel.Application myTCExcel = OpenOridnaryExcel(tclist_filename);
            if (myTCExcel != null)
            {
                Worksheet WorkingSheet = Find_Worksheet(myTCExcel, sheet_TC_Jira);
                if (WorkingSheet != null)
                {
                    ...
                    for (int index = row_tc_starting; index <= rngLast.Row; index++)  
                    {
                        ...
                        if (cell_value2 != null)
                        {
                            List<StyleString> str_list = ExtendIssueDescription(cell_value2.ToString(), global_bug_list);
        ...
        private Dictionary<string, string> global_bug_list_medical_format = new Dictionary<string, string>();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                global_bug_list = ProcessJiraBugFile(@args_str);
